...
- NAT gateway: enterprise -level VPC gateway, which provides SNAT (Source Network Address Translation) and DNAT (Destination Network Address Translation) agents for cloud platform resources, supports two network egress modes, automatic and whitelist, and provides port mapping agent services for VPC networks, making external networks Access the virtual machine through the NAT gateway.
