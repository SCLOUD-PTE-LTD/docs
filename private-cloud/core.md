...
- NAT gateway: An enterprise-level VPC gateway that provides `SNAT (Source Network Address Translation)` and `DNAT (Destination Network Address Translation)` proxies for cloud platform resources, supports automatic and whitelist network egress modes, and provides port mapping proxy services for VPC networks, enabling external networks to access virtual machines and MySQL through NAT gateways.
